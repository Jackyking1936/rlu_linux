...
class rlu_trader():
    def __init__(self, login_path, config_path):
        ...
        file_handler = logging.FileHandler("{0}/{1}.log.{2}".format(log_path, file_name, today_str), 'a', 'utf-8')
        ...
        open_time = today_date.replace(hour=9, minute=0, second=0, microsecond=0)
        self.open_unix = int(datetime.timestamp(open_time)*1000000)
        self.close_time = today_date.replace(hour=13, minute=31, second=0, microsecond=0)
                ...
                if (self.single_budget <= (self.total_budget-self.used_budget)):
                    if data['isLimitUpPrice']:
                        if data['volume'] >= self.vol_threshold:
                            ...
                            if buy_qty <= 0:
                                self.logger.info(data['symbol']+'...額度不足購買1張')
                            else:
                                self.logger.info(data['symbol']+'...委託'+str(buy_qty)+'股')
                                order_res = self.buy_market_order(data['symbol'], buy_qty, self.order_tag)
                                if order_res.is_success:
                                    self.logger.info(data['symbol']+"...市價單發送成功，單號: "+order_res.data.order_no)
                                    self.is_ordered[data['symbol']] = buy_qty
                                    self.used_budget+=buy_qty*data['price']
                                else:
                                    self.logger.error(data['symbol']+"...市價單發送失敗...")
                                    self.logger.error(str(order_res.message))
                        else:
                            self.logger.info(data['symbol']+"...交易量不足，不下單...")
                else:
                    self.logger.info(data['symbol']+" 總額度超限 "+"已使用額度/總額度: "+str(self.used_budget)+'/'+str(self.total_budget))
