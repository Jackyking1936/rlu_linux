...
class MainApp(QWidget):
    def __init__(self, active_acc):
        ...
        #清單讀取
        label_file_path = QLabel('自選清單路徑:')
        layout_condition.addWidget(label_file_path, 3, 0)
        self.lineEdit_default_file_path = QLineEdit()
        layout_condition.addWidget(self.lineEdit_default_file_path, 3, 1)
        self.folder_btn = QPushButton('')
        self.folder_btn.setIcon(QIcon('folder.png'))
        layout_condition.addWidget(self.folder_btn, 3, 2)

        self.read_csv_btn = QPushButton('')
        self.read_csv_btn.setText('讀取清單')
        self.read_csv_btn.setStyleSheet("QPushButton { font-weight: bold; }")
        layout_condition.addWidget(self.read_csv_btn, 3, 3)
    ...
    def read_target_list(self):
        ...
        if str(target_path) == '.':
            ...
            target_symbols = list(self.buy_target.iloc[:, 1].str.replace('.TW', ''))
            self.print_log("target symbols: "+str(target_symbols))
            target_symbols = list(set(target_symbols).difference(set(self.subscribed_ids.keys())))
            self.print_log("new target: "+str(target_symbols))
    ...
    def showDialog(self):
        ...
        # Open the file dialog to select a file
        if my_target_path:
            file_path, _ = QFileDialog.getOpenFileName(self, '請選擇您的下單自選清單', my_target_path, 'All Files (*)')
        else:
            file_path, _ = QFileDialog.getOpenFileName(self, '請選擇您的下單自選清單', 'C:\\', 'All Files (*)')
